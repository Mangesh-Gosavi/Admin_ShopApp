...
function Updatestock() {
    ...
    const data = {"productid":parseInt(id),"size":size,"stock":parseInt(stock)}
            try {
                const token = localStorage.getItem('token');
                console.log("Token retrieved:", token);
                const response = await fetch("http://localhost:3001/updatestock", {
                    method: "POST",
                    headers: {
    ...
    {loading ? (
       <Loader/>
        ...
        <form className='form' onSubmit={handleSubmit}>
                <label className='data'>ID:<input className='input' onChange={(e)=>setId(e.target.value)} type='number' placeholder='Enter product ID' required/></label><br/>
